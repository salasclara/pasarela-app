...
const scopeColor: Record<string, { bg: string; text: string }> = {
  Moda:    { bg: '#7B2D3E22', text: '#C9A66B' },
  Belleza: { bg: '#C4826A22', text: '#C4826A' },
  Talento: { bg: '#E8C5B022', text: '#7B2D3E' },
  Dallas:  { bg: '#C9A66B22', text: '#7B2D3E' },
};
...
export default function Feed() {
  ...
  const [catActiva, setCatActiva] = useState('Moda');
  ...
  const abrirResumen = async (noticia: Noticia) => {
    ...
    try {
      const prompt = `Eres un editor de contenido de moda y belleza para Pasarela Studio Internacional. Resume la siguiente noticia en 3-4 líneas en español, con tono elegante y sofisticado. Enfócate en tendencias, estilo y talento.\n\nTítulo: ${noticia.titulo}\nDescripción: ${noticia.descripcion}\nFuente: ${noticia.fuente}`;
  ...
  const publicarEnBlog = async () => {
    ...
    try {
      ...
      const data = await resp.json();
      if (data.url) {
        Alert.alert('✅ Publicado', 'El artículo se publicó exitosamente.', [
  ...
  const copiarTexto = async () => {
    ...
    Alert.alert('✓ Copiado', 'El artículo se copió al portapapeles.');
  };
  ...
  const compartirPor = async (app: string) => {
    ...
    if (app === 'Instagram') {
      ...
      Alert.alert('📋 Texto copiado', 'Abre Instagram y pega el texto en tu publicación.');
      return;
    }
    ...
    return (
      <ScrollView style={styles.container}>
        <TouchableOpacity style={styles.backBtn} onPress={() => setPantalla('resumen')}>
          <Ionicons name="arrow-back" size={18} color="#E8C5B0" />
          <Text style={styles.backText}>Volver al resumen</Text>
        </TouchableOpacity>
        <View style={styles.seccionRow}>
          <Text style={styles.secLabel}>Artículo redactado</Text>
        ...
        <View style={styles.contentBox}>
          {cargandoRelato ? (
            <>
              <ActivityIndicator color="#C9A66B" style={{ margin: 20 }} />
              <Text style={styles.cargandoText}>Claude está redactando...</Text>
            </>
          ) : (
        ...
        {!cargandoRelato && (
          <>
            <TouchableOpacity style={[styles.blogBtn, publicandoBlog && { opacity: 0.6 }]} onPress={publicarEnBlog} disabled={publicandoBlog}>
              {publicandoBlog ? <ActivityIndicator size="small" color="#E8C5B0" /> : <Ionicons name="globe-outline" size={16} color="#E8C5B0" />}
              <Text style={styles.blogBtnText}>{publicandoBlog ? 'Publicando...' : 'Publicar en Blog'}</Text>
            </TouchableOpacity>
            ...
            <View style={styles.shareGrid}>
              ...
              <TouchableOpacity style={[styles.shareBtn, styles.shareFull]} onPress={copiarTexto}>
                <Ionicons name="copy-outline" size={16} color="#C9A66B" />
                <Text style={[styles.shareBtnText, { color: '#C9A66B' }]}>Copiar texto</Text>
              </TouchableOpacity>
            </View>
          </>
        )}
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={styles.origTitle}>
          ...
          <View style={styles.fuenteRow}>
            <Ionicons name="newspaper-outline" size={11} color="#C4826A" />
        ...
        <View style={styles.contentBox}>
          {cargandoResumen ? (
            <>
              <ActivityIndicator color="#C9A66B" style={{ margin: 20 }} />
              <Text style={styles.cargandoText}>Claude está resumiendo...</Text>
            </>
          ) : (
        ...
        {!cargandoResumen && (
          <>
            <Text style={styles.secLabel}>Tono del artículo</Text>
            <View style={styles.tonoRow}>
              {['informativo', 'analitico', 'narrativo'].map(t => (
                <TouchableOpacity key={t} style={[styles.tonoChip, tono === t && styles.tonoChipActivo]} onPress={() => setTono(t)}>
            ...
            <TouchableOpacity style={styles.primaryBtn} onPress={generarRelato}>
              <Ionicons name="pencil-outline" size={16} color="#0D0A0B" />
              <Text style={styles.primaryBtnText}>Redactar artículo completo</Text>
            </TouchableOpacity>
          </>
        )}
      </ScrollView>
    ...
    <View style={styles.container}>
      ...
      {cargandoNoticias ? (
        <View style={styles.cargandoContainer}>
          ...
          <Text style={styles.cargandoText}>Cargando contenido...</Text>
        </View>
      ) : (
        <ScrollView style={styles.lista} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="#C9A66B" />}>
          {noticiasFiltradas.length === 0 ? (
            <Text style={styles.vacio}>No hay contenido en esta sección{'\n'}Desliza hacia abajo para actualizar</Text>
          ) : (
            noticiasFiltradas.map(n => (
              <TouchableOpacity key={n.id} style={styles.card} onPress={() => abrirResumen(n)}>
                <View style={styles.cardMeta}>
                  <View style={[styles.scopeTag, { backgroundColor: scopeColor[n.scope]?.bg || '#7B2D3E22' }]}>
                    <Text style={[styles.scopeTagText, { color: scopeColor[n.scope]?.text || '#C9A66B' }]}>{n.scope}</Text>
                  </View>
                ...
                <View style={styles.cardFooter}>
                  ...
                  <TouchableOpacity style={styles.aiBtn} onPress={() => abrirResumen(n)}>
                    <Ionicons name="sparkles-outline" size={11} color="#C9A66B" />
...
const styles = StyleSheet.create({
  ...
  scopeTag:           { borderRadius: 10, paddingHorizontal: 7, paddingVertical: 2 },
  scopeTagText:       { fontSize: 10, fontWeight: '500' },
